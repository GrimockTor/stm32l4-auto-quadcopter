...
HardwareSerial SerialLP1(LPUART1_RX, LPUART1_TX);
...
void gps_setup(void) 
{
  ...
  SerialLP1.write(Disable_GPGSV, 11);
  ...
  SerialLP1.write(Set_to_57kbps, 28);
  delay(200);

  SerialLP1.begin(57600);
  delay(200);
}

void DRONE::read_gps(void) 
{
  static uint8_t new_line_found = 0;
  uint8_t incomming_message[100], fix_type;
  ...
  uint8_t new_gps_data_available, new_gps_data_counter;
  ...
  float gps_p_gain = 2.7f;                    //Gain setting for the GPS P-controller (default = 2.7).
  float gps_d_gain = 6.5f;                    //Gain setting for the GPS D-controller (default = 6.5).
  
  while (SerialLP1.available() && new_line_found == 0) {                                                   //Stay in this loop as long as there is serial information from the GPS available.
    char read_serial_byte = SerialLP1.read();                                                              //Load a new serial byte in the read_serial_byte variable.
  ...
  if (new_line_found == 1) {                                                                             //If a new NMEA line is found.
    ...
    if (incomming_message[4] == 'L' && incomming_message[5] == 'L' && incomming_message[7] == ',') {     //When there is no GPS fix or latitude/longitude information available.
      digitalWrite(BLUE_LED, !digitalRead(BLUE_LED));                                                    //Change the LED on the STM32 to indicate GPS reception.
  ...
  if (new_gps_data_available) {                                                                           //If there is a new set of GPS data available.
    if (number_used_sats < 8)digitalWrite(BLUE_LED, !digitalRead(BLUE_LED));                              //Change the LED on the STM32 to indicate GPS reception.
    else digitalWrite(BLUE_LED, HIGH);                                                                    //Turn the LED on the STM solid on (LED function is inverted). Check the STM32 schematic.
    ...
    if (flight_mode >= 3 && waypoint_set == 1) {                                                          //If the GPS hold mode and the waypoints are stored.
      ...
      if (flight_mode == 3 && takeoff_detected == 1) {
        if (!latitude_north) {
          l_lat_gps_float_adjust += 0.0015 * (((channel[CH_2] - 1500) * cos(gps_man_adjust_heading * 0.017453)) + ((channel[CH_1] - 1500) * cos((gps_man_adjust_heading - 90) * 0.017453))); //South correction
        }
        else {
          l_lat_gps_float_adjust -= 0.0015 * (((channel[CH_2] - 1500) * cos(gps_man_adjust_heading * 0.017453)) + ((channel[CH_1] - 1500) * cos((gps_man_adjust_heading - 90) * 0.017453))); //North correction
        }

        if (!longiude_east) {
          l_lon_gps_float_adjust -= (0.0015 * (((channel[CH_1] - 1500) * cos(gps_man_adjust_heading * 0.017453)) + ((channel[CH_2] - 1500) * cos((gps_man_adjust_heading + 90) * 0.017453)))) / cos(((float)l_lat_gps / 1000000.0) * 0.017453); //West correction
        }

        else {
          l_lon_gps_float_adjust += (0.0015 * (((channel[CH_1] - 1500) * cos(gps_man_adjust_heading * 0.017453)) + ((channel[CH_2] - 1500) * cos((gps_man_adjust_heading + 90) * 0.017453)))) / cos(((float)l_lat_gps / 1000000.0) * 0.017453); //East correction
        }
      }
      ...
      gps_roll_adjust = ((float)gps_roll_adjust_north * cos(myDrone.angle_yaw * 0.017453)) + ((float)gps_pitch_adjust_north * cos((myDrone.angle_yaw - 90) * 0.017453));
      gps_pitch_adjust = ((float)gps_pitch_adjust_north * cos(myDrone.angle_yaw * 0.017453)) + ((float)gps_roll_adjust_north * cos((myDrone.angle_yaw + 90) * 0.017453));
  ...
  if (gps_watchdog_timer + 1000 < millis()) {                      //If the watchdog timer is exceeded the GPS signal is missing.
    if (myDrone.flight_mode >= 3 && myDrone.start > 0) {           //If flight mode is set to 3 (GPS hold).
      myDrone.flight_mode = 2;                                     //Set the flight mode to 2.
      myDrone.error = 4;                                           //Output an error.
    }
  }

  if (myDrone.flight_mode < 3 && waypoint_set > 0) {                                                              //If the GPS hold mode is disabled and the waypoints are set.
