...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void DRONE::start_stop_takeoff(BMP280 &bmp280) 
{
  if (channel[CH_3] < 1050 && channel[CH_4] < 1050)start = 1;                              //For starting the motors: throttle low and yaw left (step 1).
  if (start == 1 && channel[CH_3] < 1050 && channel[CH_4] > 1450) {                        //When yaw stick is back in the center position start the motors (step 2).
    ...
    bmp280.ground_pressure = bmp280.actual_pressure;                                             //Register the pressure at ground level for altitude calculations.
  ...
  if (start == 2 && channel[CH_3] < 1050 && channel[CH_4] > 1950) {
  ...
  if (takeoff_detected == 0 && start == 2) {                                       //When the quadcopter is started and no take-off is detected.
    if (channel[CH_3] > 1480 && throttle < 1750) throttle++;                           //When the throttle is half way or higher, increase the throttle.
    if (throttle == 1750)error = 6;                                                //If take-off is not detected when the throttle has reached 1700: error = 6.
    if (channel[CH_3] <= 1480) {                                                       //When the throttle is below the center stick position.
