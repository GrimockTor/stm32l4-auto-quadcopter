...
MPU9250::MPU9250(TwoWire &bus, uint8_t address){
  ...
  uint8_t loop_var;
  float temp[4];
  // good time to do the housekeeping of getting accel cal values from memory
  ...
  mpu9250.setAccelCalX(temp[0], temp[3]);
  mpu9250.setAccelCalY(temp[1], temp[4]);
}
...
void MPU9250::setup() {
  ...
  setSrd(8);    // DataRate of ~110 Hz to make sure we are above 100Hz or compass will drop to 8Hz (from 100Hz)
  // TODO???: check that getMagScaleFactorXYZ = 130uT????
}
...
#########################################
## TODO:TEST IF IT WORKS BETTER TO     ##
## CAL WITH RANGE/BW MATCHING REGULAR  ##
## OPERATION (500dps, 8g, 41Hz, 11srd) ## 
#######################################*/
